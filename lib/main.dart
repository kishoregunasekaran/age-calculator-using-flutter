...
class _HomeScreenState extends State<HomeScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      backgroundColor: Colors.yellow, // Changed background color to yellow
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            // Text widget for age category removed
          ],
        ),
      ),
    );
  ...
  calculateNextBirthday(DateTime birth) {
    ...
    if (nextBirthday.isBefore(now)) {
      ...
      if (isLeapYear(nextBirthday.year) &&
          birth.month == 2 &&
          birth.day == 29) {
        nextBirthday = DateTime(nextBirthday.year, 3, 1);
      }
    } else if (isLeapYear(nextBirthday.year) &&
        birth.month == 2 &&
        birth.day == 29) {
      nextBirthday = DateTime(nextBirthday.year, 3, 1);
    }
